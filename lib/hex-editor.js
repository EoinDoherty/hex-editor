...
import HexEditorView from './hex-editor-view';
...
export default {
  ...
  activate(state) {
    this.hexEditorView = new HexEditorView(state.hexEditorViewState);
    this.modalPanel = atom.workspace.addModalPanel({
      item: this.hexEditorView.getElement(),
      visible: false
    });
  ...
  serialize() {
    return {
      hexEditorViewState: this.hexEditorView.serialize()
    };
  },
  ...
  if (editor = atom.workspace.getActivePaneItem()) {
      ...
      if (fileTitle.slice(fileTitle.length-4, fileTitle.length) == '.hex'){
        ...
        let original = filePath.slice(0, filePath.length - 4)
        let index;
        ...
        var translatedFile = original.slice(0, index) + '(translated)' + original.slice(index, original.length);

        //storeInFile(translatedFile, decoded);
        decodeHexAndStore(selection, translatedFile);
        atom.open({'pathsToOpen':[translatedFile], newWindow: false});

      }else{
        let newHexFilePath = fileDir + fileTitle + '.hex';
        let hexText = formatHex(toHex(filePath));

        storeInFile(newHexFilePath, hexText);
        atom.open({'pathsToOpen':[newHexFilePath], newWindow: false});
      }
    }
  }
};
