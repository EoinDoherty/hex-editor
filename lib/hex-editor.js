...
function dumpHexString(hexString, ctx, addLines){
  ...
  ctx.lineNum = ctx.lineNum || 0;
  ctx.numBytes = ctx.numBytes || 0;
  ctx.line = ctx.line || "";
  ctx.linelen = ctx.linelen || 0;
  ctx.ascii = ctx.ascii || "";
  ...
  encode() {
    ...
    if (editor = atom.workspace.getActivePaneItem()){
      ...
      try {
        ...
        alert('Open the file and select "View hex dump" or use the keymap from the file\'s page.');
      }
  ...
  decode() {
    ...
    if (editor = atom.workspace.getActivePaneItem()){
      ...
      try{
        ...
        alert('Open the file and select "Decode hex dump" or use the keymap from the file\'s page.');
      }
    }
  }
};
